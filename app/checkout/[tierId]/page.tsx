...
import type { Database, SubscriptionTierFeature } from "@/lib/database.types";
        ...
        <div className="space-y-4 p-4 bg-muted/50 rounded-lg">
          ...
          <div className="space-y-2">
            ...
            <ul className="space-y-2">
              {(tier.subscription_tier_features as SubscriptionTierFeature[])?.map((feature, index) => (
                <li key={index} className="flex items-center gap-2 text-sm">
                  <Sparkles className="h-4 w-4 flex-shrink-0 text-primary" />
                  <span className={cn("text-sm", feature.comingSoon && "text-muted-foreground")}>{feature.feature}</span>
                  {feature.comingSoon && (
